...
- Embedding generation (`internal/embedding/`) — `Embedder` interface + Ollama-backed implementation (`internal/embedding/ollama/`) for `nomic-embed-text`; `embedding.NewEmbedder` factory mirrors `vectorstorage.NewClient`; shared types in `embedding/types/` break the import cycle. Query and document text are prefixed (`search_query: `/`search_document: `) when the model is `nomic-embed-text`, per its task-instruction convention — gated by model name so other embedding models aren't corrupted by a prefix that isn't theirs
